...
const QuickReminderModal = ({ isOpen, onClose }: QuickReminderModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (name.length > 50) {
      setError('Reminder name must be 50 characters or less');
      return;
    }
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="mx-auto max-w-lg w-full bg-white rounded-xl shadow-xl">
          <div className="flex items-center justify-between px-6 py-4 border-b">
            <Dialog.Title className="text-lg font-semibold text-gray-900">Add Quick Reminder</Dialog.Title>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 px-6 py-4 bg-gray-50 rounded-b-xl">
              <button
                type="button"
                onClick={onClose}
              ...
              <button
                ...
                className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-primary-500 rounded-lg hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
